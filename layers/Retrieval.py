...
class RetrievalTool():
    def __init__(
        ...
        self.topm = topm
        self.freq_top_m = max(int(freq_top_m), 0)
...
#             data_all = data_all - cur
    ...
    def _build_frequency_key(self, data_flat):
        """
        Build a frequency-domain retrieval key from flattened multivariate series.

        data_flat: (G, B/T, S*C)
        returns:   (G, B/T, F*C), where F excludes the DC bin when possible.
        """
        g, n, features = data_flat.shape
        assert features == self.seq_len * self.channels

        data = data_flat.reshape(g, n, self.seq_len, self.channels)
        freq = torch.fft.rfft(data.float(), dim=2)
        mag = torch.log1p(torch.abs(freq))

        if mag.size(2) > 1:
            mag = mag[:, :, 1:, :]

        return mag.flatten(start_dim=2)
    ...
    def periodic_batch_corr(self, data_all, key, in_bsz = 512):
        ...
        assert features == self.seq_len * self.channels
        bx = self._build_frequency_key(key)
        if bx.size(2) == 0:
            return torch.zeros(self.n_period, bsz, train_len, dtype=key.dtype, device=key.device)

        use_top_m = 0 < self.freq_top_m < bx.size(2)
        if use_top_m:
            _, query_top_idx = torch.topk(bx, k=self.freq_top_m, dim=2)
            bx_top = torch.gather(bx, 2, query_top_idx)
        else:
        ...
        for i in range(iters):
            ...
            ax = self._build_frequency_key(cur_data)

            if use_top_m:
                ax = ax.unsqueeze(1).expand(-1, bsz, -1, -1)
                ax = torch.gather(
                    ax,
                    3,
                    query_top_idx.unsqueeze(2).expand(-1, -1, end_idx - start_idx, -1),
                )
                cur_sim = F.cosine_similarity(
                    bx_top.unsqueeze(2),
                    ax,
                    dim=3,
                    eps=1e-8,
                ...
                cur_sim = torch.bmm(bx, ax.transpose(-1, -2))
            sim.append(cur_sim)
